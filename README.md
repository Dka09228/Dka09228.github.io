...
├── js/
│   ├── i18n.js       # Мультиязычность RU/EN (localStorage + автоопределение)
...
- **Мультиязычность RU / ENG** — мгновенное переключение без перезагрузки, выбор сохраняется в `localStorage`, при первом визите язык определяется по браузеру
...
4. **Английская версия — уже встроена.** Сайт поддерживает два языка (РУС / ENG); для браузеров без русского английский включается автоматически — зарубежные клиенты сразу видят англоязычную версию.
